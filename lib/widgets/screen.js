...
Screen.bind = function (screen) {
  ...
  process.on('uncaughtException', Screen._exceptionHandler = function (err) {
    //https://github.com/chjj/blessed/commit/995a8311de793ff2301f1f55fd7119c9ae910620
    //BEGIN PATCH
    /*
  ...
  for (y = start; y <= end; y++) {
    ...
    for (x = 0; x < len; x++) {
      ...
      /*
          ...
          I added a flag to cells on index 2, which indicates combo/double width characters
      */
      if (this.fullUnicode) {
        if (
          unicode.charWidth(line[x][1]) === 2 ||
          uni === true
        ) {
          line[x].length = 2;
...
Screen.prototype.readEditor = function (options, callback) {
  ...
  options = options || {};
  // https://github.com/chjj/blessed/commit/9356efc4e7c1633f8c29c134e50fc428cf9b4b44
...
/*
    ...
    also that elements can focus on a child when the parent focuses/initializes
*/
Screen.prototype._updateSelected = function (select, self) {
  if (!self.parent) {
    return;
  }
...
Screen.prototype._setUnselected = throttle(
  function (el) {
...
Screen.prototype._setSelection = function (focused, blurred) {
  if (focused) {

...
/*
  Changes to getWidth, getHeight, getLeft, getRight and position getters/setters were made
  to implement a 'positionParent' option, which takes a blessed element, 

    You can use the positionParent option to specify an element to use for relative positioning, while
    having another element as the actual parent (in terms of the node tree).

    In other words, this option lets you divorce 'positioning' from 'draw order',
    which are otherwise linked as a consequence of how Blessed renders the tree of nodes.
    The painter's algorithm is perfectly fine in most cases, but sometimes you need to do things differently.

    This is useful for 'popups' or modals, which may need to be children of 'screen' or
    an element higher in the node tree (for various reasons), but you also need them to be positioned
    relative to (within) some given element lower in the tree. Otherwise, they would need to be children
    or siblings of the related element, which is undesirable in some situations.

    For example, imagine a tmux / multiplex setup, where you need a popup to appear over a particular pane.
    In terms of draw order, this element needs to be a child of screen or a top level element, but
    in terms of positioning, it needs to be centered within the relevant window.
*/
...
Element.prototype.sattr = function (style, fg, bg) {
  ...
  bg = colors.convert(bg, 'bg');

  if (darken > 0 && bg > 0) {
    bg = colors.darken(bg, darken, 'bg');
  }

  if (darken > 0) {
    fg = colors.darken(fg, darken, 'fg');
  } else {
    fg = colors.convert(fg, 'fg');
  }
...
Element.prototype._parseAttr = function (lines) {
  ...
  // https://github.com/chjj/blessed/pull/306  
  // PATCH BEGIN 
  /*
...
*/
Element.prototype.__defineGetter__('hidden', function () {
...
Element.prototype.__defineSetter__('hidden', function (value) {
  this._hidden = value;
});
...
Element.prototype.setLabel = function (options, style) {
  var self = this;
  var Box = require('./box');
...
Element.prototype.setPos = function (newPos, emit = true, clear = true) {
  let move = false;
  let resize = false;
...
Element.prototype.isInside = function (x, y) {
...
Element.prototype.isOutside = function (x, y) {
  if (this.hidden || this.detached) {
    return false;
  }
